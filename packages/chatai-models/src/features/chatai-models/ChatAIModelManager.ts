...
type KnownProducer = 'openai' | 'google' | 'anthropic' | 'vertexai' | 'debug';
...
class ChatAIModelManager {
    ...
    #loadDefaultModel() {
        const openAIProvider = this.#addProvider('OpenAI', 'openai');
        initOpenAIProvider(openAIProvider);

        const geminiProvider = this.#addProvider('Google', 'google');
        initGeminiProvider(geminiProvider);

        const claudeProvider = this.#addProvider('Anthropic', 'anthropic');
        initClaudeProvider(claudeProvider);

        const vertexAIProvider = this.#addProvider('VertexAI', 'vertexai');
