...
class Profile {
    ...
    static async From(profilePath: string | null, required: ProfileRequried, logger?: LevelLogger) {
        const profile = new Profile(profilePath, required, logger);
        await profile.initialize();
        return profile;
    }

    private constructor(profilePath: string | null, required: ProfileRequried, logger?: LevelLogger) {
        ...
        this.#storage.addAccessEvent('secret-json', {
            async init(fullPath, tree) {
                const masterKey = required.masterKeyGetter.masterKey;
    ...
    async #readPersonalKey(): Promise<string> {
        const masterKey = this.#masterKeyGetter.masterKey;
        if (!masterKey) throw new ProfileError('Master key is not initialized');
    ...
    async createUsingTemplate(metadata: RTMetadata, templateId: string) {
        this.logger.info(`Create new RT using template: ${templateId} (${metadata.id})`);
