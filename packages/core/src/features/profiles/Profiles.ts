...
import type { IMasterKeyGettable } from '@/features/masterkey-manager/types';

type ProfilesRequried = {
...
class Profiles {
    ...
    static async From(basePath: string | null, required: ProfilesRequried) {
        const instance = new Profiles(basePath, required);
        await instance.loadMetadata();
        return instance;
    }

    private constructor(basePath: string | null, required: ProfilesRequried, logger?: LevelLogger) {
        this.#basePath = basePath;
        this.#logger = logger ?? NoLogger.instance;
        this.#masterKeyGetter = required.masterKeyGetter;
        ...
        this.#storage.addAccessEvent('profile', {
            async init(fullPath) {
                return Profile.From(fullPath, { masterKeyGetter: required.masterKeyGetter });
            },
            async save(ac) {
                await ac.commit();
            }
    ...
    async loadMetadata() {
        ...
        if (profiles === undefined) {
            this.#logger.debug(`Profiles metadata not found`);
        }
        profiles ??= [];
        last_profile ??= null;
        next_profile_id ??= 0;
        ...
        this.#logger.info(`Profiles loaded: ${this.#profileIdentifiers.length} profiles`);
    }
    ...
    async saveMetadata() {
        ...
        this.#logger.info(`Profiles saved: ${this.#profileIdentifiers.length} profiles`);
    }
    ...
    async createProfile() {
        ...
        this.#logger.info(`Profile created: ${identifier}`);
    ...
    async deleteProfile(identifier: string) {
        ...
        await accessor.drop();
        this.#logger.info(`Profile deleted: ${identifier}`);
    ...
    async getOrphanProfileIds(): Promise<string[]> {
        ...
        this.#logger.debug(`Searching for orphan profiles in ${this.#basePath}...`);
    ...
    async recoverOrphanProfile(orphanProfileId: string) {
        this.#logger.trace(`Recovering orphan profile: ${orphanProfileId}`);
        if (this.#existsProfileId(orphanProfileId)) {
            this.#logger.info(`Skipping recovery for existing profile: ${orphanProfileId}`);
            return;
        }
        if (!this.testProfileValidity(orphanProfileId)) {
            this.#logger.info(`Failed to recover orphan profile: ${orphanProfileId} (validity check failed)`);
            return;
        }
        ...
        this.#logger.info(`Recovered orphan profile: ${orphanProfileId}`);
    }

    private testProfileValidity = (profileId: string) => {
        this.#logger.trace(`Checking profile validity: ${profileId}`);
        if (!this.#basePath) {
            this.#logger.trace(`InMemory mode: skipping profile validity check`);
            return false;
        }

        const profilePath = path.join(this.#basePath, profileId, 'config.json');
        if (!fs.existsSync(profilePath)) {
            this.#logger.trace(`Profile config file not found: ${profilePath}`);
            return false;
        }
        ...
        try {
            ...
            if (!profileData.name) {
                this.#logger.trace(`Profile config is invalid: missing 'name' field`);
                return false;
            }
        }
        catch (error) {
            this.#logger.trace(`Error reading profile config: ${error}`);
            return false;
        }

        this.#logger.trace(`Profile '${profileId}' is valid`);
        return true;
    }


    async saveAll() {
        this.#logger.info(`Saving all profiles...`);
        await this.saveMetadata();
