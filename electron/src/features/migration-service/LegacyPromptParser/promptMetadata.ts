...
export class PromptMetadata implements IPromptMetadata {
    ...
    async loadPromptTemplate() {
        ...
        try {
            const template = await PromptFS.loadPromptTemplate(this.#path, this.#basePath);
            if (template.startsWith("@FAIL")) {
                throw new Error(`Fail to load prompt template : ${this.#path}\n\n${template}`);
            }
            else {
